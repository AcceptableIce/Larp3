...
<html>
	<head>
		...
		if($authed) {
			...
			if($theme && $theme != "default") echo "<link href='/css/themes/$theme.css' rel='stylesheet' type='text/css'>";
		}
		?>
	</head>
	<body>
		<nav id="primary-nav" class="top-bar" data-topbar role="navigation">
			...
			<section class="top-bar-section">
				<ul class="left">
					...
					@if(!$authed)
						<li>
							<a href="/contact">Contact the Storytellers</a>
						</li>
					@endif
				</ul>
				@if($authed)
				 	...
				 	<li data-tooltip class="hide-for-small" title="Report Bug"><a href="/forums/35/post"><i class="icon-bug"></i></a></li>
		      		<li class="has-dropdown">
		        		<a href="#">
							...
							@if($unread > 0) 
								<label class="label round radius success topbar-mail-alert">{{$unread}}</label> 
							@endif
						</a>
		        		<ul class="dropdown">
		          			<li>
		...
		@if($authed && Auth::user()->hasPermission('View Debug Data')) 
			<div class="page-build-time">
				Page built in {{round((microtime(true) - $_SERVER["REQUEST_TIME_FLOAT"])*1000, 3)}}ms
			</div>
		@endif
