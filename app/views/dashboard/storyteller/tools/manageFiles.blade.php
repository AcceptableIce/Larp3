	...
	@if($mode == "manage")
		...
		<table class="responsive">
			...
			<tbody>
				...
				<tr>
					<td>
						<a href="/dashboard/storyteller/manage/files/{{$upload->id}}/edit">
							<i class="icon-pencil"></i>
						</a>
						<a href="/dashboard/storyteller/manage/files/{{$upload->id}}/delete">
							<i class="icon-trash"></i>
						</a>
					</td>
					<td>
						<a href="/uploads/{{$upload->url}}">{{$upload->name}}</a>
					</td>
					<td>
						{{$upload->read_permission ? $upload->readPermission->name : ""}}
					</td>
					<td>
		...
		<form method="post" action="/dashboard/storyteller/manage/files/upload" enctype="multipart/form-data">
				...
				<div class="small-10 columns">
					<select name="permission">
						...
						@foreach(PermissionDefinition::all() as $p)
							<option value="{{$p->id}}" {{$file && $file->read_permission == $p->id ? 'selected' : ''}}>
								{{$p->name}}
							</option>
						@endforeach
					</select>
			...
			<div class="row">
				...
				<div class="small-10 columns">
					...
					@if($file)
						<p>
							Not selecting a file will keep the already-uploaded file.
						</p>
					@endif
				</div>
			</div>
