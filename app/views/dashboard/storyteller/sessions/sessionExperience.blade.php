...
<div class="row left">
	@if(isset($id))
		...
		<table class="responsive">
			...
			<tbody data-bind="foreach: $root.activeCharacters">
				<tr>
					...
					<td>
						<input type="text" name="overrides[]" class="checkin-experience-box" data-bind="value: override" />
					</td>
					<td style="text-align: center;" 
						data-bind="text: (costume() ? 1 : 0) + (nomination1() ? 1 : 0) + (nomination2() ? 1 : 0) + parseInt(override(), 10) + 1">
					</td>
				</tr>
			</tbody>
		</table>
		...
		@else
			<p>
				<b>Experience for this session has already been awarded.</b>
			</p>
		@endif
		</form>
	@else
				...
				<tr>
					...
					<td>
						@if($d->submitted)
							Awarded
						@else
							{{$d->checkins->count() > 0 ? "<b>Pending Approval</b>" : "<i>Incomplete</i>"}}
						@endif
					</td>
					<td>
						<a href="/dashboard/storyteller/session/experience/{{$d->id}}">
							<button class="button small success">Award Experience...</button>
						</a>
					</td>
				</tr>
				@endforeach
