...
<div class="row left">
	...
	<button class="button tiny delete-toggle" style="left: 350px;" 
			data-bind="click: function() { $root.showDeletes(!$root.showDeletes()) }, 
					   text: $root.showDeletes() ? 'Enable Deleting' : 'Disable Deleting'"></button>
	<p>	
		Deleting a category will unassociate all forums which belong to it.<br>
		...
		<tbody>
			@foreach(ForumCategory::orderBy('display_order')->get() as $d)
				<tr>
					...
					<td>
						<form method="post" action="/dashboard/storyteller/manage/forums/categories/update">
					...
					<td>
						<form method="post" action="/dashboard/storyteller/manage/forums/categories/remove">
