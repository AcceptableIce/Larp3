...
<div class="row left">
	...
	<p>Clicking on one of buttons below will award experience for that month. Any box with a checkmark in it has already been awarded.<br> <b>Journal experience cannot be taken back.</b></p>
	<table class="journal-grid responsive">
		...
		<tbody>
			...
			<tr>
				...
				@foreach($months as $m) 
				<td>
				@if(CharacterJournalExperience::where('character_id', $c->id)->whereRaw('MONTH(date) = ?', [date('m', $m)])->whereRaw('YEAR(date) = ?', [date('Y', $m)])->exists())
