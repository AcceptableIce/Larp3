...
<div class="row storyteller-content left">
	...
	<div class="small-12 medium-7 columns">
		...
		<? 	$topics = ForumTopic::where('is_complete', 0)->whereHas('forum', function($q) { 
				$q->where('show_on_st_todo_list', true); 
			})->orderBy('created_at', 'desc')->get(); ?>
		<p>
			There are currently <b>{{$topics->count()}}</b> incomplete threads. 
			@if($topics->count() > 0) 
				The oldest thread was created <b>{{$topics->last()->created_at->diffForHumans()}}</b>.
			@endif
		</p>
				...
				@foreach($topics as $topic)
					<tr>
						<td class="st-incomplete-row">
							<div class="unread-topics {{$topic->hasUnreadPosts($user->id) ? 'unread' : ''}}"></div>
						</td>
						<td>
							<a href="/forums/topic/{{$topic->id}}">{{$topic->title}}</a>
						</td>
						<td>
	...
	<div class="small-12 medium-5 columns">
		...
		<? $topics = ForumTopic::where('is_complete', 0)
						->where(['forum_id' => 35, 'is_complete' => false, 'is_sticky' => false])
			...
			<tbody>
				@foreach($topics as $topic)
					<tr>
						...
						<td>
							{{str_replace('ago' ,'', $topic->firstPost->created_at->diffForHumans())}}
						</td>
					</tr>
				@endforeach
			</tbody>
