...
<div class="row left">
	...
	<p>	
		Clicking on one of buttons below will award diablerie Experience for that month. 
		Any box with a checkmark in it has already been awarded.<br> 
		<b>Awarding diablerie Experience cannot be undone.</b>
	</p>
	<table class="journal-grid responsive">
		...
		<tbody>
			@foreach(Character::where(['is_npc' => false, 'in_review' => false, 'active' => true])
					->where('approved_version', '>', 0)->orderBy('name')->get() as $c)
				<tr>
					<td>
						{{$c->name}}
					</td>
					@foreach($months as $m) 
						<td>
							@if(CharacterDiablerieExperience::where('character_id', $c->id)
