...
<div class="row left">
	...
	<button class="button tiny delete-toggle" data-bind="click: function() { $root.showDeletes(!$root.showDeletes()) }, text: $root.showDeletes() ? 'Enable Deleting' : 'Disable Deleting'"></button>
	<form method="post" action="/dashboard/storyteller/influence/caps/update">
		...
		@foreach(InfluenceCap::with('definition')->get()->sortBy('definition.name') as $cap)
			<tr>
				...
				<td><input type="text" class="small-input" name="capacities[]" value="{{$cap->capacity}}" /></td>
				<td>
					<select name="deltas[]">
					@foreach(['+','','-'] as $v)
	...
	<form method="post" action="/dashboard/storyteller/influence/caps/add" class="panel">
		...
		<select name="background">
			@foreach(RulebookBackground::where('group', 'Influence')->get() as $bg)
				@if(!InfluenceCap::where('background_id', $bg->id)->exists())<option value="{{$bg->id}}">{{$bg->name}}</option>@endif
			@endforeach
		</select>
