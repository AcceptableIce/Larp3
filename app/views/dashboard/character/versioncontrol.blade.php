...
<div id="delete-modal" class="reveal-modal" data-reveal aria-labelledby="deleteModalTitle" aria-hidden="true" role="dialog">
  <h2 id="deleteModalTitle">Revert to Version <span data-bind="text: promptName"></span>?</h2>
  <p>Are you sure you wish to revert {{$character->name}} to verison <span data-bind="text: promptName"></span>? 
  <b>This cannot be undone. All versions after version <span data-bind="text: promptName"></span> will be deleted.</b></p>
  <p>Type this character's name into the box below to confirm the revert.</p>
...
<table>
	...
	@foreach($character->versions()->orderBy('version', 'desc')->get() as $version)
		<tr>
			<td>
				<a href="/dashboard/character/{{$character->id}}/print/{{$version->version}}"><i class="icon-doc-text-inv" data-tooltip title="View Character Sheet"></i></a>
			</td>
			<td>{{$version->version}} 
			...
			<td>
				@if($version->version == $character->approved_version || Auth::user()->isStoryteller()) 
					...
					<button class="button small alert" data-bind="click: function() { prompt({{$version->version}}) }">Force to Version {{$version->version}}</button>
					@elseif($version->version != $character->latestVersion()->version)
					<button class="button small alert" data-bind="click: function() { prompt({{$version->version}}) }">Revert to Version {{$version->version}}</button>
					@endif
				@endif
			</td>
		</tr>
