function chargenVM() {
		...
		self.getMaximumRitualRank = function() {
			...
			for(var i in self.characterSheet.disciplines()) {
				...
				if(disc.name == "Thaumaturgy" || disc.name == "Necromancy" || disc.name == "Countermagic") if(disc.count > limit) limit = disc.count;
			}
		...
		self.hasThaumaturgy = function() {
			return _.findIndex(self.characterSheet.disciplines(), function(item) { return item.name == "Thaumaturgy" || item.name == "Countermagic"; }) !== -1;
		}
